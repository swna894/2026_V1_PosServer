...
public Product handleProductNotFound(SaleItemRequest request) {
    ...
    if (barcode.startsWith("QUICK_")) {
        try {
            Product newProduct = createQuickProduct(request);
            System.err.println("✅ 임시 상품 생성됨: " + newProduct);
            return newProduct;  // 이미 저장된 상태로 반환
        } catch (Exception e) {
 ...
 */
    private Product createQuickProduct(SaleItemRequest request) {
        System.err.println("📦 createQuickProduct 시작 - Request: " + request);
        ...
        System.err.println("📝 임시 상품 정보 - code: " + tempCode + 
        ...
        try {
            ...
            // 💡 Category 엔티티와 연동하려면 아래처럼 별도 setter 필요
            // product.setCategory(category);  // Product 엔티티에 category 필드가 Category 타입이어야 함
            
            System.err.println("🏷️ Product 생성 완료 (저장 전): " + product);
            
        } catch (IllegalArgumentException e) {
            throw ExceptionUtils.invalidInput("product data", e.getMessage());
        }
        
        // Product 저장
        Product savedProduct = productRepository.save(product);
        System.err.println("💾 Product 저장 완료 - ID: " + savedProduct.getId());
        ...
        System.err.println("📦 Stock 생성 완료 - ID: " + savedStock.getId() + 
                        ", 재고량: " + savedStock.getQuantity());
        
        // ===================================================
        // Step 6: (선택사항) 강제 flush로 DB 반영 확인
        // ===================================================
        entityManager.flush();
        System.err.println("✅ DB 반영 완료 (flush)");
        
        // ===================================================
        // Step 7: StockHistory 기록 (선택사항)
        // ===================================================
        // 💡 재고 변동 이력을 남기려면 아래 코드 활성화
        /*
        StockHistory history = StockHistory.of(
            savedProduct,
            0,                           // 변동 수량 (0)
            StockHistory.StockType.IN,   // 입고 타입
            "INITIAL_STOCK"              // 참조값
        );
        stockHistoryRepository.save(history);
        System.err.println("📜 재고 이력 기록 완료");
        */
